...
import { IsDateString, IsEnum, IsNotEmpty, IsString, MaxLength, Matches } from 'class-validator';
...
export class CreateChildRequest {
  ...
  @IsEnum(Gender, { message: `gender 은(는) ${Object.values(Gender).join(', ')} 중 하나여야 합니다.` })
  @IsNotEmpty({ message: `gender 은(는) 필수 입력 항목입니다.` })
  gender: Gender;
  ...
  @IsEnum(Profile, { message: `profile 은(는) ${Object.values(Profile).join(', ')} 중 하나여야 합니다.` })
  @IsNotEmpty({ message: `profile 은(는) 필수 입력 항목입니다.` })
  profile: Profile;
}
