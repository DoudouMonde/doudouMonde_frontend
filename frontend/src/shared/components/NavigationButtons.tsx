...
const NavigationButtons: React.FC<NavigationButtonsProps> = ({
  ...
  return (
    <div className={`grid grid-cols-2 gap-2 w-full ${className}`}>
      ...
      <button
        onClick={onPrevious}
        className="flex items-center justify-center gap-2 w-full h-[34px] bg-secondary-100 rounded-[20px]  hover:bg-secondary-100/80 hover:border-x-secondary-100/50 transition-all duration-200"
      >
      ...
      <button
        ...
        className={`flex items-center justify-center gap-2 w-full h-[34px] rounded-[20px] transition-all duration-200 active:scale-95 ${
          isNextDisabled
            ...
            : "text-gray-200 bg-green-100 hover:scale-105 body-inter-r"
        }`}
      >
