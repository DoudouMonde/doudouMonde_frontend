...
import { Profile } from '@doudoumonde/shared/schemas';
...
export const childApi = {
  ...
  getChildDetail: async (childId: number) => {
    const res = await apiRequester.get<ChildDetailResponse>(`/v1/child/${childId}`);

    return res.data;
  },
  patchChild: async (childId: number, payload: UpdateChildRequest) => {
    //response는 확인 후 수정
    const res = await apiRequester.patch<childVoidResponse>(`/v1/child/${childId}`, payload);
    if (res.status !== 200) throw new Error('아이 정보 수정 실패');
    return res.data;
  },

  deleteChild: async (childId: number) => {
    const res = await apiRequester.delete<childVoidResponse>(`/v1/child/${childId}`);
